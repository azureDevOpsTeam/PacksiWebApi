...
public class BlockUserCommandHandler(IUserAccountServices userAccountServices, IMiniAppServices miniAppServices, ILiveChatServices liveChatServices, ILogger<GetUsersListQueryHandler> logger) : IRequestHandler<BlockUserCommand, HandlerResult>
{
    ...
    public async Task<HandlerResult> Handle(BlockUserCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = await _liveChatServices.BlockUserAsync(request.Model, userAccount.Value);
