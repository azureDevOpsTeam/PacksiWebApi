...
public class GetRecentConversationsQueryHandler(IUserAccountServices userAccountServices, IMiniAppServices miniAppServices, ILiveChatServices liveChatServices, ILogger<GetUsersListQueryHandler> logger) : IRequestHandler<GetRecentConversationsQuery, HandlerResult>
{
    ...
    public async Task<HandlerResult> Handle(GetRecentConversationsQuery request, CancellationToken cancellationToken)
    {
        ...
        var result = await _liveChatServices.GetRecentConversationsAsync(userAccount.Value);
