...
public class MiniApp_UpdateRequestHandler(IUnitOfWork unitOfWork, IUserAccountServices userAccountServices, IMiniAppServices miniAppServices) : IRequestHandler<MiniApp_UpdateRequestCommand, HandlerResult>
{
    private readonly IUserAccountServices _userAccountServices = userAccountServices;
    private readonly IMiniAppServices _miniAppServices = miniAppServices;
    private readonly IUnitOfWork _unitOfWork = unitOfWork;

    public async Task<HandlerResult> Handle(MiniApp_UpdateRequestCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var resultAddRequest = await _miniAppServices.UpdateRequestAsync(request, userAccount.Value, cancellationToken);
            ...
            var resultItemtype = await _miniAppServices.UpdateRequestItemTypeAsync(request, resultAddRequest.Value.Id);
